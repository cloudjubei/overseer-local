...
##### Step 8d — MergeConflictResolver _(shipped 2026-05-12)_
...
#### Step 13 — Delete redundant local primitives _(shipped 2026-05-12)_

- **Migrated remaining stragglers first.** The Step 3–12 list missed `screens/projects/` (ProjectManagerModal, ProjectEditorForm, ProjectGroupsEditor, ProjectCodeInfoModal, ProjectWizard*, CodeInfoChip), `screens/LiveDataView.tsx`, `screens/ChatView.tsx`'s `CollapsibleSidebar` import, and the agents row composites (AgentRunRow, CostChip, StatusChip, ProjectChip, TokensChip). Swapped `Button`, `Modal`, `Spinner`, `Switch`, `Tooltip`, `DotBadge`, `CollapsibleSidebar` + a handful of icons across those files. Select usage remained on the local Radix composite (deferred — see section D).
- **Deleted 24 dead primitives** under [src/renderer/src/components/ui/](../src/renderer/src/components/ui/): `Alert.tsx`, `BranchChip.tsx`, `Button.tsx`, `Chip.tsx`, `Code.tsx`, `CollapsibleSidebar.tsx`, `DiffViewer.tsx`, `DotBadge.tsx`, `FileDisplay.tsx`, `Input.tsx`, `JsonView.tsx`, `Markdown.tsx`, `Markdown.css`, `Modal.tsx`, `PathDisplay.tsx`, `ResizeHandle.tsx`, `SafeText.tsx`, `SegmentedControl.tsx`, `Skeleton.tsx`, `Spinner.tsx`, `SpinnerWithDot.tsx`, `Switch.tsx`, `Toast.tsx`, `Tooltip.tsx`, `TypewriterText.tsx`. Bundle dropped ~14 KB JS / ~2 KB CSS.
- **Kept** under `components/ui/`:
  - **Wrappers** around package primitives: `FileMentionsTextarea.tsx`, `FileSelector.tsx`, `RichText.tsx` — these wire local `useFiles()` / `useStories()` data into the package primitives. Keep until either the package gains a default-resolver convention or the wrappers become trivial pass-throughs.
  - **Project-specific composites**: `CommandMenu.tsx`, `ContextInfoButton.tsx`, `ErrorBubble.tsx`, `DiagnosticsOverlay.tsx`, `ShortcutsHelp.tsx` — no package equivalent, app-specific behaviour.
  - **Deferred swap**: `Select.tsx` (Radix composite — see section D).
  - **Icons** under `components/ui/icons/`: not deleted. [projectIcons.tsx](../src/renderer/src/screens/projects/projectIcons.tsx) star-imports the whole barrel (`import * as Icons from '@renderer/components/ui/icons/Icons'`) to power the project-icon picker, and many of those icons are project-decoration-only (medical / sports / construction / finance series) that don't belong in `thefactory-ui`. A future cleanup pass can replace per-file `IconX from '../ui/icons/Icons'` imports with package imports where applicable, but the local barrel stays so the picker keeps working.

#### Step 14 — Retire the local design-token CSS _(shipped 2026-05-12)_

- **Resolved the prerequisite Open Question first.** The `:root` merge gotcha (where the package's tokens.css declarations partially disappeared from the consumer's compiled CSS) was a symptom of the old `@import` chain in [index.css](../src/renderer/src/index.css) — Vite inlined the local `@import './styles/...'` files before PostCSS saw them, and the layering with the package's own `:root` blocks ended up with non-overlapping keys silently dropping. Restructuring `index.css` into a 2-line shim with each local CSS file imported as a JS module in [main.tsx](../src/renderer/src/main.tsx) (the same fix that silenced the `@import must precede` warnings) makes each `:root` block its own module-graph node, and they merge in the browser the way CSS specifies. Verified by removing the manually-patched `--surface-muted` from the local file and confirming the package's value reaches the bundle.
- **Audited all design tokens.** Diff between [styles/design-tokens.css](../src/renderer/src/styles/design-tokens.css) (now deleted) and [thefactory-ui/src/web/styles/tokens.css](../../thefactory-ui/src/web/styles/tokens.css) showed: zero local-only tokens, one package-only token (`--surface-muted`). Resolved values matched (local used `var(--color-gray-50)` indirection; package used the literal hex — same final value).
- **Diffed the rest.** Every component / foundation / primitive / layout / utility CSS file under `styles/` was byte-identical to its package counterpart, except `components/chat.css` where the package was a strict superset (45 extra lines of `.markdown-body` rules).
- **Deleted 17 files:** [design-tokens.css](../src/renderer/src/styles/design-tokens.css), [foundations/metrics.css](../src/renderer/src/styles/foundations/metrics.css), [primitives/effects.css](../src/renderer/src/styles/primitives/effects.css), [components/{buttons,forms,feedback,badges,tooltip,overlays,cards,segmented,file-display,file-mentions,chat,standard-picker}.css](../src/renderer/src/styles/components/), [layout/nav.css](../src/renderer/src/styles/layout/nav.css), [utilities/semantic-utilities.css](../src/renderer/src/styles/utilities/semantic-utilities.css). Empty directories removed too.
- **Kept:** [styles/app.css](../src/renderer/src/styles/app.css) (body / html / h1-h3 / .empty — app-level rules) and the five project-specific screen sheets under [styles/screens/](../src/renderer/src/styles/screens/) (`stories`, `story-details`, `board`, `docs`, `settings`).
- **Bundle:** CSS dropped from 218 KB to 164 KB (-54 KB / -25 %). JS unchanged.
...
- **CSS pipeline restructured to silence PostCSS strict-mode `@import must precede` warnings.** _2026-05-12._ Two changes: (1) the malformed `var(transparent)` rules in [styles/screens/story-details.css](../src/renderer/src/styles/screens/story-details.css) (the `.rejection-badge` placeholder) corrected to plain `transparent` keywords — `var()` only accepts custom-property names (`--name`). (2) Restructured how CSS gets into the renderer: `index.css` is now a 2-line shim (`@import 'tailwindcss'; @import 'thefactory-ui/web/styles';`) and every local stylesheet is imported as a JS module in [main.tsx](../src/renderer/src/main.tsx) instead of via `@import` chains. Vite inlines `@import` chains before PostCSS sees them, so inlined local rules ended up before the trailing `@import 'tailwindcss'` / `@import 'thefactory-ui/web/styles'` — every later @import was flagged. Splitting them across JS imports keeps each CSS file in its own module graph and `index.css` only contains @imports. App-level rules (body, html, .empty) live in [styles/app.css](../src/renderer/src/styles/app.css). Upstream fix in `thefactory-ui` paired: the tsup `onSuccess` step now appends the Tailwind `@source` directive at the END of `dist/styles/index.css` rather than prepending it (was triggering the same warning at the bundle's first @import).

- **`resolve.dedupe: ['react', 'react-dom']` required in [electron.vite.config.ts](../electron.vite.config.ts).** _2026-05-12._ `thefactory-ui` is linked via `file:` (`node_modules/thefactory-ui` is a symlink to the sibling repo) and the sibling repo has its own `node_modules/react` from its dev/build deps. Vite resolves bare `react` imports from the importer's nearest `node_modules`, so without dedupe the renderer ends up bundling **two** React copies — the consumer's and the package's. Hooks fail with "Cannot read properties of null (reading 'useState')" on any package component that uses React state (`ToastProvider` was the first to crash). Surfaced during Step 13 verification. Adding `resolve.dedupe` forces a single React tree; bundle also drops ~37 KB as the duplicate disappears. Keep this in mind for any future `file:`-linked package in this repo.
...
_(empty — `RichText` is now Step 5d, `useToast/ToastProvider` is Step 5e, `FileSelector` is Step 6a, `FileMentionsTextarea` is Step 6b, Radix `Select` lifted in Step 14 follow-up.)_
