...
- src/: Application source (Electron app: main, preload, renderer, and tooling).
  ...
  - logic/factory/: Main process integration with the 'thefactory-tools' library — agent runs, completion, pricing, and the test runner. Also the integration point for the library's CLI agent runner (`runner: 'cli'` on chat dispatch + story execution; Docker-sandboxed Claude Code today, Cursor / Codex pending) and for the two auth flows the library exposes (subscription device-flow login + paste-and-encrypt API key).
