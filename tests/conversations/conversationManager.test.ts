...
describe('conversations/conversationManager', () => {
  ...
  it('should call auth helpers to configure the backend client', async () => {
    vi.mocked(ConversationsService.conversationsControllerHandle).mockResolvedValue({ type: 'UNKNOWN' } as any);
  ...
  it('should handle an unknown response type and NOT clear state', async () => {
    ...
    expect(mockBot.sendMessage).not.toHaveBeenCalled();
    expect(result).toEqual(expect.objectContaining({ type: 'error', error: { message: 'Unexpected conversation response.', retry: true } }));
  });
  ...
  it('should build an empty input if message text is missing', async () => {
    ...
    expect(ConversationsService.conversationsControllerHandle).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('should not send messages if chatId is missing', async () => {
    ...
    const backendResponse = { type: ConversationResponseDto.type.SUCCESS, success: { message: 'test' } };
    vi.mocked(ConversationsService.conversationsControllerHandle).mockResolvedValue(backendResponse as any);
