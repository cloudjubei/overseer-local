...
describe('lib/auth', () => {
  ...
  describe('handleAuthMessage', () => {
    ...
    it('should handle successful login with a valid access code', async () => {
      ...
      expect(setSession).toHaveBeenCalledWith(expect.objectContaining({ userId, accessToken: 'new-token' }));
      expect(setAccessToken).toHaveBeenCalledWith('new-token');
      expect(mockBot.sendMessage).toHaveBeenCalledWith(mockMsg.chat.id, 'You are now authenticated. 🎉');
    });
    ...
    it('should handle failed login with an invalid access code', async () => {
      ...
      expect(mockBot.sendMessage).toHaveBeenCalledWith(mockMsg.chat.id, expect.stringContaining('did not work'));
    });
