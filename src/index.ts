...
import quickSuggestionAction, { chooseSuggestion, quickSuggestionAction2 } from './actions/quickSuggestion'
...
import { sendCategoryKeyboardMessage, sendDifficultyKeyboardMessage, sendTypeKeyboardMessage } from './common/keyboards'
...
import actionMacroGoal, { clearMacroSuggestionsForMessage, getMacroSuggestionsForMessage, processMacroInput } from './actions/actionMacroGoal'
...
async function runOnboardingIfNeeded(bot: TelegramBot, msg: Message, rawText: string): Promise<boolean> {
  const { chat, from } = msg
  ...
  try {
    ...
    if (data.startsWith('profile:gender:')) {
      ...
      try {
        ...
        await ProfilesService.profilesControllerUpdate({ requestBody: { gender: UserProfileModel.gender[genderKey] as any } })
    ...
    if (data.startsWith('q:')) {
      ...
      if (data.startsWith('q:type:cat:diff:')) {
        ...
        try {
          ...
          await bot.sendMessage(chatId, typeof errMsg === 'string' ? errMsg : 'Failed to get suggestions.')
        }
        return
      }
    ...
    // Handle suggestion actions (legacy quick suggestions)
