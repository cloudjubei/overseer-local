...
function normalizeGenderInput(text: string): Gender | undefined {
  ...
  if (t === 'other' || t === 'o' || t === 'non-binary' || t === 'nonbinary' || t === 'nb') return 'OTHER';
  return undefined;
}

export async function handleProfileFlowMessage(bot: TelegramBot, userId: string, msg: TelegramBot.Message) {
  const state = flows.get(userId);
    ...
    case 'ask_gender': {
      if (!skipped && text.length > 0 && !text.startsWith('/')) {
        ...
        if (g) {
          ...
          await bot.sendMessage(chatId, 'Please choose one of: Male, Female, Other. Or reply with skip.', {
            reply_markup: {
              keyboard: [[{ text: 'Male' }, { text: 'Female' }, { text: 'Other' }], [{ text: 'skip' }]],
              resize_keyboard: true,
              one_time_keyboard: true,
    ...
    case 'ask_height': {
      ...
      try {
        ...
        if (result) {
          ...
          const weightPart = (result as any).weight ? `${(result as any).weight} kg` : (result as any).weight_raw ? `${(result as any).weight_raw}` : undefined;
          if (weightPart) parts.push(`Weight: ${weightPart}`);
          const heightPart = (result as any).height ? `${(result as any).height} cm` : (result as any).height_raw ? `${(result as any).height_raw}` : undefined;
