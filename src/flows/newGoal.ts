...
export async function handleNewGoalFlowMessage(bot: TelegramBot, userId: string, msg: TelegramBot.Message) {
  const state = flows.get(userId);
  if (!state) return;
  ...
  if (state.step === 'ask_text' || state.step === 'await_refine') {
    ...
    try {
      ...
      await bot.sendMessage(state.chatId, 'Sorry, I could not get suggestions right now. Please try again later.');
...
export async function handleNewGoalCallback(
  ...
  if (pickMatch) {
    ...
    try {
      ...
      state.step = 'showing_suggestions';
      flows.set(userId, state);
      await bot.sendMessage(chatId, 'Sorry, I could not create that goal. Please try another suggestion or refine your message.');
    }
    return true;
  }
