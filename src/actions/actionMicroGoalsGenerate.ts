  ...
  if (!skipEnergyLevels) {
    try {
      ...
      if (!hasTodayEnergy) {
        ...
        // NEW: Send motivation text before showing energy options
        try {
          ...
          const latest = lifestyles.length > 0 ? lifestyles[lifestyles.length - 1] as any : undefined
          const motivationText = typeof latest?.motivationText === 'string' ? latest.motivationText.trim() : ''
          if (motivationText) {
