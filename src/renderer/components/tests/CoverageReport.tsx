...
function buildTooltip({ covered, total }: { covered: number | null | undefined; total: number | null | undefined }) {
  if (typeof covered === 'number' && typeof total === 'number') return `${covered}/${total}`
  return ''
}

function MetricCell({ label, pct, covered = null, total = null, showLabel = true }: MetricCellProps) {
  const pctVal = typeof pct === 'number' ? pct : null
  const title = buildTooltip({ covered, total })
  return (
    <div className="w-28 mx-auto text-center" title={title}>
      {showLabel ? (
        <div className={`text-xs font-medium ${pctVal !== null ? 'text-neutral-600 dark:text-neutral-300' : 'text-neutral-400'}`}>{label}</div>
      ) : null}
    ...
    <div className="overflow-auto border rounded-md border-neutral-200 dark:border-neutral-800">
      <table className="min-w-full text-sm table-fixed">
        ...
        <thead className="bg-neutral-50 dark:bg-neutral-800/50 text-neutral-600 dark:text-neutral-400">
          ...
          <tr className="border-t border-neutral-200 dark:border-neutral-700/50">
            ...
            <th className="text-center px-3 pt-2 pb-3 font-normal">
              <MetricCell label="Statements" pct={summary.avgStatementsPct} showLabel={false} />
            </th>
            <th className="text-center px-3 pt-2 pb-3 font-normal">
              <MetricCell label="Branches" pct={summary.avgBranchesPct} showLabel={false} />
            </th>
            <th className="text-center px-3 pt-2 pb-3 font-normal">
              <MetricCell label="Functions" pct={summary.avgFunctionsPct} showLabel={false} />
            </th>
            <th className="text-center px-3 pt-2 pb-3 font-normal">
              <MetricCell label="Lines" pct={summary.avgLinesPct} showLabel={false} />
            </th>
