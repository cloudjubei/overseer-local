...
export default function AgentRunRow({
  ...
  // Optimistic local rating + animation state
  const [localRating, setLocalRating] = useState<AgentRunRatingPatch | undefined>(run.rating)
  const [isAnimating, setIsAnimating] = useState(false)
  const [animKind, setAnimKind] = useState<'up' | 'down' | null>(null)

  // Keep local rating in sync when props change (unless we are mid animation pop)
  useEffect(() => {
    if (!isAnimating) setLocalRating(run.rating)
  }, [run.rating, isAnimating])

  const handleRate = (rating: AgentRunRatingPatch | undefined) => {
    // Optimistic local change
    setLocalRating(rating)
    // Trigger animation when setting a rating (not when clearing)
    if (rating) {
      setIsAnimating(true)
      setAnimKind(rating.score === 1 ? 'up' : 'down')
      // End animation after 700ms
      window.setTimeout(() => {
  ...
  const displayedRating = localRating
        ...
        <td className="w-16">
          {!displayedRating ? (
            <div className="flex flex-col items-center gap-1">
              ...
              >
                <span className={`inline-flex items-center justify-center rating-effect ${isAnimating && animKind === 'up' ? 'rating-pop rating-pop--up' : ''}`}>
              ...
              <button
                ...
                disabled={isAnimating}
              >
                <span className={`inline-flex items-center justify-center rating-effect ${isAnimating && animKind === 'down' ? 'rating-pop rating-pop--down' : ''}`}>
