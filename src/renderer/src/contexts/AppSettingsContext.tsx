...
import { AppSettings, NotificationSystemSettings, UserPreferences } from 'src/types/settings'
import { DEFAULT_APP_SETTINGS } from '@renderer/utils/settings'
...
export function AppSettingsProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateAppSettings = useCallback(
    async (updates: Partial<AppSettings>): Promise<AppSettings> => {
  ...
  const setUserPreferences = useCallback(
    async (updates: Partial<UserPreferences>): Promise<AppSettings> => {
      const next = {
  ...
  const setNotificationSystemSettings = useCallback(
    async (updates: Partial<NotificationSystemSettings>): Promise<AppSettings> => {
      const next = {
