    ...
    >
      ...
      <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
        <Tooltip content={'reset (discard local changes)'} placement="bottom">
          <Button variant="ghost" size="sm" onClick={() => onReset(file)}>
            RESET
          </Button>
        </Tooltip>
...
export default function GitCommitModal({
  ...
  const [divider, setDivider] = React.useState<number>(() =>
    Math.max(260, Math.floor(window.innerWidth * 0.22)),
  )
  ...
  const load = React.useCallback(async () => {
    ...
    setError(undefined)
    try {
      const s: any = await gitService.getLocalStatus(
        projectId as any,
        { includePatch: true } as any,
      )
  ...
  const reset = async (paths: string[]) => {
    const ok = window.confirm(
      'Discard all local changes to the selected file(s)? This cannot be undone.',
    )
    if (!ok) return
    setBusy(true)
    setOpError(undefined)
  ...
  const onDragStart =
    (kind: 'staged' | 'unstaged' | 'untracked', path: string) => (e: React.DragEvent) => {
    ...
    <>
      <Modal isOpen={true} onClose={onRequestClose} title={header} size="xl" footer={footer}>
        <div className="relative flex w-full min-h-[420px] max-h-[70vh]">
          {/* Left panel */}
          <div
            className="shrink-0 border-r border-neutral-200 dark:border-neutral-800 overflow-hidden"
      ...
      >
        <div className="space-y-3">
          <div className="text-sm text-neutral-700 dark:text-neutral-300">
            Commit {stagedCount} file{stagedCount === 1 ? '' : 's'} on{' '}
