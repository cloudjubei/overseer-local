...
export default function MergeConflictResolver({
  ...
  const isVeryLarge = React.useMemo(
    () => (content?.length || 0) > 200000 || segments.length > 800,
  ...
  const loadTriples = React.useCallback(
  ...
  const loadFile = React.useCallback(
    async (path: string | null) => {
      ...
      try {
        ...
        const hasLF = /(?<!\r)\n/.test(data)
        const eolType: '\n' | '\r\n' | 'mixed' =
          hasCRLF && hasLF ? 'mixed' : hasCRLF ? '\r\n' : '\n'
        originalEolByPathRef.current[path] = eolType
        setEolWarning(
          eolType === 'mixed' ? 'File has mixed line endings. Consider normalizing.' : null,
        )
        ...
        setEncodingWarning(
          looksMojibake
            ? 'File may have non-UTF-8 encoding. Saving as UTF-8 could alter bytes.'
            : null,
        )
  ...
  React.useEffect(() => {
    if (selected) void loadTriples(selected, baseRef, branch)
  ...
  const onSave = async () => {
    ...
    try {
      ...
      toast({ title: 'File saved', description: selected })
    } catch (e: any) {
  ...
  const conflictIdxList = React.useMemo(
    () => segments.map((s, i) => (s.type === 'conflict' ? i : -1)).filter((i) => i >= 0),
    [segments],
  )
  ...
  React.useEffect(() => {
    setFocusedHunk(conflictIdxList[0] ?? -1)
  }, [selected])
  ...
  React.useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (!inText && (e.key === 'ArrowDown' || e.key === 'ArrowUp')) {
        ...
        if (e.key === 'ArrowDown') {
          const next =
            conflictIdxList[Math.min(conflictIdxList.length - 1, Math.max(0, curPos) + 1)]
  ...
  const doFinalize = React.useCallback(async () => {
    ...
    try {
      ...
      const unresolved = Object.entries(resolvedMap)
        .filter(([_, v]) => !v)
        .map(([k]) => k)
      if (unresolved.length > 0) {
        toast({
          title: 'Cannot finalize',
          description: `${unresolved.length} file(s) still have unresolved markers.'}`,
        })
        return
      }
      try {
      ...
      const message =
        finalizeMsg && finalizeMsg.trim().length > 0 ? finalizeMsg : `Merge branch ${branch}`
      ...
      toast({ title: 'Merge finalized', description: `Committed ${conflicts.length} file(s).` })
    } catch (e: any) {
      console.error('Finalize merge failed', e)
      toast({
        title: 'Finalize merge failed',
        description: String(e?.message || e),
        variant: 'error',
      })
    } finally {
  ...
  const leftPaneRef = React.useRef<HTMLPreElement | null>(null)
  const rightPaneRef = React.useRef<HTMLPreElement | null>(null)
  const onSyncScroll = React.useCallback(
    (src: 'left' | 'right') => {
    ...
    <div className="flex flex-col gap-3 min-h-[480px]">
      ...
      <div className="flex items-center justify-end gap-3 text-xs">
        ...
        <label className="inline-flex items-center gap-1">
          <input
            ...
            onChange={(e) => setNormalizeEol(e.target.checked)}
          />
              ...
              <Tooltip content={'open in external merge tool'} placement="bottom">
                <span>
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        await factoryToolsService.executeTool(
                          projectId,
          ...
          {!selected ? (
            ...
            <div className="p-4 text-sm text-neutral-600 dark:text-neutral-400">
              ...
              <div className="mt-2 flex items-center gap-2">
                <Button size="sm" onClick={() => void acceptFile('ours')}>
                  Accept ours
                </Button>
                <Button size="sm" onClick={() => void acceptFile('theirs')}>
                  Accept theirs
                </Button>
            ...
            <div className="p-4 text-sm text-neutral-600 dark:text-neutral-400">
              Large file detected. For performance, hunk rendering is disabled. Use file-level
              actions below and Save & Stage.
              <div className="mt-2 flex items-center gap-2">
                <Button size="sm" onClick={() => void acceptFile('ours')}>
                  Accept ours (file)
                </Button>
            ...
            <div className="flex-1 min-h-0 overflow-auto p-3 flex flex-col gap-3">
              ...
              {/* 3-pane context */}
              <div
                className={`grid grid-cols-1 ${showContextPanes ? 'lg:grid-cols-3' : 'lg:grid-cols-1'} gap-3`}
              >
                {showContextPanes && (
                  <div className="min-w-0">
                    ...
                    {triplesLoading ? (
                      ...
                      <pre
                        ...
                        className={`text-xs font-mono ${wrapLines ? 'whitespace-pre-wrap break-words' : 'whitespace-pre'} max-h-64 overflow-auto border border-neutral-200 dark:border-neutral-800 rounded-md`}
                      >
              ...
              {segments.some((s) => s.type === 'conflict') && (
                <div className="flex items-center gap-2">
                  <Tooltip content={'accept all ours/current'} placement="bottom">
                    <span>
                      <Button
                        variant="secondary"
                        size="sm"
                        onClick={() => applyChoiceAll('ours')}
                        aria-label="Accept all ours"
                      >
                        Accept ours (all)
                      </Button>
                    </span>
                  ...
                  <Tooltip content={'accept all theirs/incoming'} placement="bottom">
                    <span>
                      <Button
                        variant="secondary"
                        size="sm"
                        onClick={() => applyChoiceAll('theirs')}
                        aria-label="Accept all theirs"
                      >
                        Accept theirs (all)
                      </Button>
                    </span>
                  ...
                  <Tooltip content={'concatenate both sides for all hunks'} placement="bottom">
                    <span>
                      <Button
                        variant="secondary"
                        size="sm"
                        onClick={() => applyChoiceAll('both')}
                        aria-label="Accept both for all hunks"
                      >
                        Accept both (all)
                      </Button>
                    </span>
                  ...
                  <div className="ml-auto flex items-center gap-2">
                    <Button variant="ghost" size="sm" onClick={() => void acceptFile('ours')}>
                      Accept ours (file)
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => void acceptFile('theirs')}>
                      Accept theirs (file)
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => void resetFileToOriginal()}>
                      Reset file
                    </Button>
                  </div>
                </div>
              ...
              {segments.map((seg, idx) => {
                if (seg.type === 'text') {
                  return (
                    <div
                      key={idx}
                      className="border rounded-md border-neutral-200 dark:border-neutral-800"
                    >
                      <pre
                        className={`text-xs font-mono ${wrapLines ? 'whitespace-pre-wrap break-words' : 'whitespace-pre'} max-h-64 overflow-auto`}
                      >
                  ...
                  >
                    ...
                    <div className="p-2 grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        <div className="mt-2">
                          <Button
                            size="sm"
                            onClick={() => applyChoice(idx, 'ours')}
                            aria-label={`Accept ours for hunk ${idx + 1}`}
                          >
                            Accept ours
                          </Button>
                        </div>
                      ...
                      <div>
                        ...
                        <div className="mt-2">
                          <Button
                            size="sm"
                            onClick={() => applyChoice(idx, 'theirs')}
                            aria-label={`Accept theirs for hunk ${idx + 1}`}
                          >
                            Accept theirs
                          </Button>
                        </div>
                    ...
                    <div className="px-3 pb-3">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => applyChoice(idx, 'both')}
                        aria-label={`Accept both for hunk ${idx + 1}`}
                      >
                        Accept both
                      </Button>
                    </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="px-2 py-1 text-xs rounded border border-neutral-200 dark:border-neutral-800 bg-transparent"
      ...
      {externalChanged && (
        <div className="fixed bottom-3 left-1/2 -translate-x-1/2 z-20 bg-amber-50 dark:bg-amber-900/40 text-amber-800 dark:text-amber-200 border border-amber-300 dark:border-amber-700 rounded-md shadow px-3 py-2 text-xs flex items-center gap-3">
          ...
          <Button size="sm" variant="ghost" onClick={() => setExternalChanged(false)}>
            Dismiss
          </Button>
        </div>
      )}
