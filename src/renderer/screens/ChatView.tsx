...
export default function ChatView() {
  const { currentChatId, setCurrentChatId, chatsById, createChat, deleteChat, sendMessage, isThinking } = useChats()
  const { configs, activeConfigId, activeConfig, isConfigured, setActive } = useLLMConfig()
  const { navigateView } = useNavigator()

  const currentChat = useMemo(() => (currentChatId ? chatsById[currentChatId] : undefined), [currentChatId, chatsById])

  const chatHistories = useMemo(() => Object.values(chatsById).sort((a, b) => new Date(a.updateDate).getTime() - new Date(b.updateDate).getTime()), [chatsById])
      ...
      <section className="flex-1 flex flex-col w-full h-[720px] bg-[var(--surface-base)] overflow-hidden">
        <header className="flex-shrink-0 px-4 py-2 border-b border-[var(--border-subtle)] bg-[var(--surface-raised)] flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => navigateView('Settings')} className="btn-secondary" aria-label="Open Settings">
              Settings
            </button>
          </div>
        </header>
        ...
        {!isConfigured && (
          <div
            ...
            style={{ background: 'color-mix(in srgb, var(--accent-primary) 10%, var(--surface-raised))', color: 'var(--text-primary)' }}
            role="status"
          >
        ...
        <MessageList chatId={currentChat?.id} messages={currentChat?.messages || []} isThinking={isThinking} />
