...
const ToolsScreen: React.FC = () => {
  ...
  const handleExecute = async () => {
    ...
    try {
      const result = await factoryService.executeTool(selectedTool.name, args);
  ...
  const renderArgInput = (key: string, schema: z.ZodTypeAny) => {
      ...
      return (
        <div key={key} className="flex items-center">
          <input
            ...
            className="mr-2"
          />
          <label htmlFor={key} className="text-sm text-gray-300">{description}</label>
        </div>
      );
    }
    ...
    return (
      <div key={key}>
        <label htmlFor={key} className="block text-sm font-medium text-gray-300 mb-1">{description}</label>
        <input
          id={key}
            ...
            <div className='bg-gray-800 p-4 rounded-lg flex flex-col flex-grow'>
              ...
              <div className="space-y-4 mb-4">
                {selectedTool.arguments && Object.keys(selectedTool.arguments.shape).length > 0 ? (
                    Object.entries(selectedTool.arguments.shape).map(([key, schema]) => renderArgInput(key, schema))
                ) : (
              ...
              <div className="mt-4 flex-grow flex flex-col">
                ...
                <div className="bg-gray-900 p-4 rounded-lg mt-2 flex-grow overflow-y-auto">
                  ...
                  {executionError && <pre className="text-red-500 whitespace-pre-wrap">{executionError}</pre>}
