...
function Frame({ children }: { children: ReactNode }) {
  return (
    <div style={{ padding: 16, background: 'var(--surface, transparent)' }}>{children}</div>
  );
}
...
export function createDefaultMocksRegistry() {
  ...
  registry.register('router', asWrapper((node) => <RouterProvider>{node}</RouterProvider>));

  // A subtle frame wrapper; can be opted-in by declaring 'frame'
  registry.register('frame', asWrapper((node) => <Frame>{node}</Frame>));
  ...
  registry.register('tasksMock', asWrapper((node) => <TasksProvider>{node}</TasksProvider>));

  // Notifications mock
  const NotificationsContext = React.createContext<{ items: any[]; push: (n: any) => void } | null>(null);
  function NotificationsProvider({ children }: { children: ReactNode }) {
    const [items, setItems] = React.useState<any[]>([]);
    const push = (n: any) => setItems((s) => [...s, { id: String(Date.now()), ...n }]);
    return <NotificationsContext.Provider value={{ items, push }}>{children}</NotificationsContext.Provider>;
  }
  ;(globalThis as any).__PreviewNotificationsContext = NotificationsContext;
  registry.register('notificationsMock', asWrapper((node) => <NotificationsProvider>{node}</NotificationsProvider>));
  ...
  registry.register('llmMock', asWrapper((node) => <LLMConfigProvider>{node}</LLMConfigProvider>));
