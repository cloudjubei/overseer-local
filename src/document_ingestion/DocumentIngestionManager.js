  ...
  handleFileAdded = async ({ projectId, relPath }) => {
    try {
      ...
      if (
        existing &&
        existing.metadata?.contentHash === contentHash &&
        existing.metadata?.mtime === f.mtime
      ) {
  ...
  handleFileRenamed = async ({ projectId, relPathSource, relPathTarget }) => {
    ...
    try {
      ...
      if (db.archiveDocument) {
        try {
          await db.archiveDocument({ id: oldId, reason: 'renamed', newPath: relPathTarget })
        } catch {}
      } else if (db.deleteDocument) {
        try {
          await db.deleteDocument({ id: oldId })
        } catch {}
      }
    } catch {}
