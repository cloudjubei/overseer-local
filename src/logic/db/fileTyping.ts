...
import type { FileMeta } from 'thefactory-tools'
...
export function isTextForIngestion(filename: string, extension?: string, type?: string): boolean {
  const ext = (extension ?? filename.split('.').pop() ?? '').toLowerCase()
  if (ext.length > 0 && (TEXT_EXTS.has(ext) || DEFAULT_CODE_EXTS.has(ext))) return true
  if (type) {
    if (type.startsWith('text/')) return true
    const t = type.toLowerCase()
    if (t.includes('json') || t.includes('xml') || t.includes('markdown')) return true
