...
export default class DocumentIngestionManager extends BaseManager {
  ...
  async ingestProject(projectId: string): Promise<void> {
    ...
    // Only consider text files for ingestion
    const textFiles = files.filter((f) => isTextForIngestion({ ext: f.ext, type: f.type, size: f.size }))
    ...
    for (const f of textFiles) {
      try {
        ...
        if (!stats || !isTextForIngestion({ ext: stats.ext, type: stats.type, size: stats.size })) continue
  ...
  private async handleFileAdded(projectId: string, relPath: string) {
    try {
      const stats = await this.filesManager.getFileStats(projectId, relPath)
      if (!stats || !isTextForIngestion({ ext: stats.ext, type: stats.type, size: stats.size })) return
