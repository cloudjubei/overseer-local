...
const NODE_BUILTINS = new Set([
  'fs','path','os','http','https','zlib','stream','crypto','child_process','worker_threads','electron','node:fs','node:path','node:os','node:http','node:https','node:zlib','node:stream','node:crypto','node:child_process'
]);
...
function analyzeExports(sf, checkerless = true) {
  ...
  sf.forEachChild(n => {
    if (ts.isFunctionDeclaration(n) && n.modifiers && n.modifiers.some(m => m.kind === ts.SyntaxKind.DefaultKeyword)) {
  ...
  sf.forEachChild(n => {
    if (ts.isVariableStatement(n) && n.modifiers && n.modifiers.some(m => m.kind === ts.SyntaxKind.ExportKeyword)) {
      n.declarationList.declarations.forEach(d => {
        ...
        let jsx = false; let props = []; let propsIncomplete = true;
        if (d.initializer && (ts.isArrowFunction(d.initializer) || ts.isFunctionExpression(d.initializer))) {
          ...
          props = res.props; propsIncomplete = res.propsIncomplete;
        }
        exports.push({ exportName: name, returnsJSX: jsx, props, propsIncomplete });
      });
    }
    if (ts.isFunctionDeclaration(n) && n.modifiers && n.modifiers.some(m => m.kind === ts.SyntaxKind.ExportKeyword)) {
...
function readPreviewMeta(sf) {
  ...
  sf.forEachChild(n => {
    if (ts.isVariableStatement(n)) {
      const isExport = n.modifiers && n.modifiers.some(m => m.kind === ts.SyntaxKind.ExportKeyword);
      if (!isExport) return;
      n.declarationList.declarations.forEach(d => {
        if (ts.isIdentifier(d.name) && d.name.text === 'preview' && d.initializer && ts.isObjectLiteralExpression(d.initializer)) {
...
function analyzeImports(sf) {
  ...
  sf.forEachChild(n => {
    if (ts.isImportDeclaration(n)) {
      const moduleName = n.moduleSpecifier && ts.isStringLiteral(n.moduleSpecifier) ? n.moduleSpecifier.text : null;
...
function analyzeFile(filePath) {
  ...
  if (status !== 'blocked' && anyIncomplete) {
    reasons.push('Props type could not be fully inferred (external or implicit). Provide props via preview URL or export a preview meta.');
  }
