      ...
      while (active < maxConcurrency && index < authenticated.length) {
        ...
        bot
          ...
          .catch((err) => {
            ...
            console.warn(`scheduler: failed to send to ${chatId} (${label})`, err?.response?.body || err?.message || err);
          })
          .finally(() => {
            active--;
            resolved++;
...
export function initScheduler(bot: TelegramBot) {
  ...
  const evening = cron.schedule('0 19 * * *', () => broadcastHello(bot, 'evening'), { timezone: tz });
