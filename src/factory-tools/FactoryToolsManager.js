...
export class FactoryToolsManager
{
  ...
  async init() {
    ...
    console.log('[factory] Pricing manager initialized. Loaded', this.PRICING?.listPrices()?.prices?.length || 0, 'prices.');
  }
  ...
  _registerIpcHandlers() {
    ...
    const handlers = {};
    handlers[IPC_HANDLER_KEYS.FACTORY_RUNS_START_TASK] = async ({ agentType, projectId, taskId, llmConfig, githubCredentials, webSearchApiKeys, options }) => this.startTaskRun(agentType, projectId, taskId, llmConfig, githubCredentials, webSearchApiKeys, options);
    handlers[IPC_HANDLER_KEYS.FACTORY_RUNS_START_FEATURE] = async ({ agentType, projectId, taskId, featureId, llmConfig, githubCredentials, webSearchApiKeys, options }) => this.startFeatureRun(agentType, projectId, taskId, featureId, llmConfig, githubCredentials, webSearchApiKeys, options);
    handlers[IPC_HANDLER_KEYS.FACTORY_RUNS_CANCEL] = async ({ runId, reason }) => await this.cancelRun(runId, reason);
  ...
  {
    console.log('[factory] START_TASK', this._maskSecrets({ agentType, projectId, taskId, llmConfig, githubCredentials, webSearchApiKeys, options }));
    try {
      const { runHistory, runHandle } = this.orchestrator.startRun({ agentType, projectId, taskId, llmConfig, githubCredentials, webSearchApiKeys, options });
  ...
  {
    console.log('[factory] START_FEATURE', this._maskSecrets({ agentType, projectId, taskId, featureId, llmConfig, githubCredentials, webSearchApiKeys, options }));
    try {
      const { runHistory, runHandle } = this.orchestrator.startRun({ agentType, projectId, taskId, featureId, llmConfig, githubCredentials, webSearchApiKeys, options });
