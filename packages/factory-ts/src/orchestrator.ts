...
async function runConversation(opts: {
  ...
  // Use a 'system' role to make the renderer display the system prompt properly
  const sysMsg: CompletionMessage = { role: 'system', content: systemPrompt } as any;
  const messages: CompletionMessage[] = [sysMsg];
  ...
  for (let i = 0; i < MAX_TURNS_PER_FEATURE; i++) {
    try {
      ...
      const assistant = { role: 'assistant' as const, content: res.message.content, durationMs } as any;
      ...
      // Append tool results as a dedicated tool message; renderer groups these as the turn's response
      const toolResultMsg: CompletionMessage = { role: 'tool', content: toolOutputs.map(o => `Tool ${o.name} returned: ${typeof o.result === 'string' ? o.result : JSON.stringify(o.result)}`).join('\n') } as any;
